    ...
    try
      ...
      //files := ;
        ...
        begin
          ...
          begin
            FTP.Put(Job.GetJob(i).FileList[j].FileDir +'\'+Job.GetJob(i).FileList[j].FileName, '/'+ DirOut + Job.GetJob(i).FileList[j].FileName);
            templ_str:=templ_str+'[Success]: ' + Job.GetJob(i).FileList[j].FileDir  +' '+Job.GetJob(i).FileList[j].FileName +' =>> '+ DirOut + Job.GetJob(i).FileList[j].FileName + '; '; //нужна проверка целостности отправл€емого файла
          end;
